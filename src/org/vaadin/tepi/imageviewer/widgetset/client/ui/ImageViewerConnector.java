...
import org.vaadin.tepi.imageviewer.widgetset.client.ui.VImageViewer.ImageSelectionListener;
...
public class ImageViewerConnector extends AbstractComponentConnector implements
        SimpleManagedLayout, ImageSelectionListener {

    private ImageViewerServerRpc rpc = RpcProxy.create(
            ImageViewerServerRpc.class, this);
    ...
    @Override
    public void layout() {
        boolean heightUpdated = getWidget().updateHeight(
                getLayoutManager().getOuterHeight(getWidget().getElement()));
        boolean widthUpdated = getWidget().updateWidth(
                getLayoutManager().getOuterWidth(getWidget().getElement()));
